...
export class AiService {
  ...
  private readonly allProgrammeNames: Map<
    string,
      ...
      if (turn.role === 'user') {
        ...
        if (text.includes('faculty of') || text.includes('school of')) {
          const match = text.match(
            /(?:faculty|school)\s+of\s+([a-z\s]+)/i,
          );

          if (match) {
            profile.preferredFaculty = match[1].trim();
          }
        }

        if (turn.programmes && turn.programmes.length > 0) {
          profile.lastProgrammes = turn.programmes.map((p) => ({
            id: p.id,
            name: p.name,
            code: p.code,
          }));
  ...
  private async extractEntities(
    ...
    const gradePattern =
      /\b([a-z\s]+?)\s*(?:\(|\s)?([a-fA-F][0-9]?|[1-9])\s*(?:\))?/g;
    ...
    while ((match = gradePattern.exec(allUserText)) !== null) {
      ...
      for (const cachedSubject of this.allSubjectsCache) {
        if (
          subjectName.includes(cachedSubject) ||
          cachedSubject.includes(subjectName)
        ) {
          if (!entities.subjects.includes(cachedSubject)) {
            entities.subjects.push(cachedSubject);
          }

          entities.subjectsWithGrades.push({
            name: cachedSubject,
            grade,
          });
    ...
    if (allUserText.includes('undergraduate')) {
      entities.levels.push('UNDERGRADUATE');
    }

    if (allUserText.includes('postgraduate')) {
      entities.levels.push('POSTGRADUATE');
    }

    if (
      allUserText.includes('doctorate') ||
      allUserText.includes('phd')
    ) {
      entities.levels.push('DOCTORATE');
    }

    if (allUserText.includes('professional')) {
      entities.levels.push('PROFESSIONAL');
    }

    if (
      allUserText.includes('bachelor') ||
      allUserText.includes('bsc') ||
      allUserText.includes('ba ') ||
      allUserText.includes('beng') ||
      allUserText.includes('bed')
    ) {
      entities.levels.push('UNDERGRADUATE');
    }

    if (
      allUserText.includes('master') ||
      allUserText.includes('msc') ||
      allUserText.includes('ma ')
    ) {
      entities.levels.push('POSTGRADUATE');
    }
    ...
    for (const deg of degreePatterns) {
      if (allUserText.includes(deg)) {
        entities.degrees.push(deg.toUpperCase());
      }
    }
    ...
    for (const word of careerKeywords) {
      if (allUserText.includes(word)) {
        entities.careerKeywords.push(word);
      }
    }

    if (/\b(first|the first|1st)\b/i.test(allUserText)) {
      entities.pronounRef = 1;
    } else if (/\b(second|the second|2nd)\b/i.test(allUserText)) {
      entities.pronounRef = 2;
    } else if (/\b(third|the third|3rd)\b/i.test(allUserText)) {
      entities.pronounRef = 3;
    } else if (
      /\b(this|that|this one|that one|it)\b/i.test(allUserText)
    ) {
      entities.pronounRef = 'this';
    } else if (
      /\b(the last|the previous)\b/i.test(allUserText)
    ) {
      entities.pronounRef = 'last';
    }

    const durMatch = allUserText.match(
      /(\d+)\s*years?\s*(program|programme|course|degree)?/i,
    );

    if (durMatch) {
      entities.durations.push(parseInt(durMatch[1], 10));
    }
  ...
  private detectIntent(
    ...
    if (
      /^(hi|hello|hey|good\s*(morning|afternoon|evening)|greetings)\b/.test(
        q,
      )
    ) {
      return 'greeting';
    ...
    if (
      /^(i don't know|not sure|undecided|what should i|recommend|suggest|what can i|what are my options|help me choose|what is out there|explore|show me everything|i have no idea|what programmes|all programmes)/.test(
        q,
      )
    ) {
      return 'undecided';
    ...
    if (
      entities.pronounRef !== 'none' &&
      profile.lastProgrammes.length > 0
    ) {
    ...
    if (
      (/compare|difference|versus|vs|better|which (one|program|programme).*(better|best|suitable)|recommend|or\s+/.test(
        q,
      ) &&
        entities.programmeCodes.length >= 2)
    ) {
      return 'comparison';
    }

    if (
      /why\s+(can'?t\s+i|don'?t\s+i|not|didn'?t|am\s+i\s+not|is\s+it)/i.test(
        q,
      ) &&
      profile.lastEligibilityResult
    ) {
      return 'explain_decision';
    }

    if (
      entities.careerKeywords.length > 0 ||
      /career|job|work as|become|profession|occupation|dream|want to be/i.test(
        q,
      )
    ) {
    ...
    if (
      entities.programmeNames.length > 0 ||
      entities.programmeCodes.length > 0
    ) {
      if (
        /tell me about|what is|details|info|show|about|requirements|entrance/.test(
          q,
        ) ||
        entities.programmeNames.length > 0
      ) {
    ...
    if (entities.subjects.length > 0) {
      if (
        /eligible|qualify|can i|requirements|subjects|i have|i took|i studied|i did|what can i/.test(
          q,
        )
      ) {
        return 'eligibility';
  ...
  private handleGreeting(profile: UserProfile): AiResponse {
    if (profile.stage !== 'new') {
      return {
        message:
          '👋 Hello again! How can I help you continue exploring programmes?',
        suggestions: [
...
• Or just browsing to see what's available?`,
      ...
      clarificationQuestions: [
        ...
        'Do you have subjects you want to work with?',
        'Are you looking for career guidance?',
      ],
    };
  }
...
Just type naturally, like you're talking to a friend!`,
  ...
  private async handleSubjectSearch(
    ...
    if (subjects.length === 0) {
      return {
        message:
          'I noticed you mentioned some subjects but I couldn\'t identify them clearly. Could you tell me which subjects you studied? For example:\n\n• "I have **Biology, Chemistry** and **Mathematics**"\n• "I studied **Physics, Economics** and **Geography**"',
        suggestions: [
          'I have Biology, Chemistry and Mathematics',
          'I studied Physics, Economics and Geography',
          'My subjects are English, History and French',
        ],
    ...
    if (profile.preferredLevel) {
      filtered = programmes.filter(
        (p: any) => p.level === profile.preferredLevel,
      );
    }
    ...
    return {
      message: `🎯 **Great!** With subjects in **${subjects.join(', ')}**, here are some programmes you might be interested in:\n\n${topProgrammes
        .map(
          (p: any, i: number) =>
            `${i + 1}. **${p.name}** (${p.code}) — ${
              p.department?.academicUnit?.abbreviation || ''
            } — ${p.duration} years`,
        )
        .join('\n')}\n\n${
        filtered.length > 6
  ...
  private async handleCareerSearch(
    entities: EntityStore,
    query: string,
    profile: UserProfile,
  ): Promise<AiResponse> {
  ...
  private async handleFacultySearch(
    entities: EntityStore,
    query: string,
    profile: UserProfile,
  ): Promise<AiResponse> {
    ...
    if (!facultyAbbr) {
      ...
      return {
        message: `🏛️ **Faculties & Schools**\n\nHere are all the academic units:\n\n${faculties
          .map(
            (f) =>
              `• **${f.name}** (${f.abbreviation}) — ${f._count.departments} departments`,
          )
    ...
    return {
      message: `🏛️ **${faculty.name} (${faculty.abbreviation})**\n${
        ...
        .map(
          (d) =>
            `• **${d.name}** (${d._count.programmes} programmes)`,
        )
  ...
  private async handleProgrammeDetail(
    entities: EntityStore,
    query: string,
    profile: UserProfile,
  ): Promise<AiResponse> {
    ...
    if (!programme) {
      return {
        message:
          'I couldn\'t find a programme matching that. Could you try searching by programme code (like **MBBS-01** or **BSC-CS-01**) or full name?',
    ...
    const tuitionStr = tuition
      ? `${Number(tuition.amount).toLocaleString()} ${tuition.currency} / year`
      : 'Contact university';
      ...
      message: `🎓 **${programme.name} (${programme.code})**\n\n📋 **Key Information:**\n• **Degree:** ${
        ...
        programme.requirements.length > 0
          ? `📚 **Subject Requirements:**\n${programme.requirements
              .map(
                (r: any) =>
                  `• ${r.subject.name} (${r.requirementType}${
                    r.minimumGrade
                      ? ` - Min: ${r.minimumGrade}`
                      : ''
                  })`,
  ...
  private async handleEligibilityQuery(
    ...
    const subjectsWithGrades =
      entities.subjectsWithGrades.length > 0
        ? entities.subjectsWithGrades
        : profile.subjectsWithGrades;
    ...
    const eligibilityResult = await this.processEligibility(
      subjects,
      subjectsWithGrades,
      profile.preferredLevel,
    );
    ...
    if (eligibilityResult.conditional.length > 0) {
      ...
      message += eligibilityResult.conditional
        .slice(0, 3)
        .map(
          (p) =>
    ...
    if (eligibilityResult.notEligible.length > 0) {
      ...
      message += eligibilityResult.notEligible
        .slice(0, 5)
        .map(
          (p) =>
            `• **${p.name}** (${p.code}) — ${p.faculty}\n  ❌ Missing: ${p.missing.join(
    ...
    if (
      ...
      eligibilityResult.conditional.length === 0
    ) {
    ...
    const allProgs = [
      ...eligibilityResult.eligible.map((p) => ({
        ...p,
        relevance: '✅ Eligible',
      })),
      ...eligibilityResult.conditional.map((p) => ({
        ...p,
        relevance: '⚠️ Partial',
      })),
      ...eligibilityResult.notEligible.map((p) => ({
        ...p,
        relevance: '❌ Not Eligible',
      })),
    ];
  ...
  private async handleExplainDecision(
    entities: EntityStore,
    query: string,
    profile: UserProfile,
  ): Promise<AiResponse> {
    ...
    if (!lastResult) {
      return {
        message:
          'I don\'t have any previous eligibility results to explain. Could you tell me your subjects first?\n\nExample: "I have Biology, Chemistry and Geography"',
        suggestions: [
    ...
    for (const prog of allProgrammes) {
      if (
        q.includes(prog.name.toLowerCase()) ||
        q.includes(prog.code.toLowerCase())
      ) {
    ...
    if (!targetProgramme && allProgrammes.length > 0) {
      const medMatch = q.match(
        /medicine|mbbs|nursing|engineering|computer\s*science|law/i,
      );
      ...
      if (medMatch) {
        const found = allProgrammes.find(
          (p) =>
            p.name
              .toLowerCase()
              .includes(medMatch[0].toLowerCase()) ||
            p.code
              .toLowerCase()
              .includes(medMatch[0].toLowerCase()),
        );
        ...
        if (found) {
          targetProgramme = found.name;
        }
      }
    }

    if (targetProgramme) {
      const eligible = lastResult.eligible.find(
        (p) => p.name === targetProgramme,
      );

      const conditional = lastResult.conditional.find(
        (p) => p.name === targetProgramme,
      );

      const notEligible = lastResult.notEligible.find(
        (p) => p.name === targetProgramme,
      );
      ...
      if (notEligible) {
        ...
        if (
          programmeDetails &&
          programmeDetails.requirements.length > 0
        ) {
          detailsSection = `\n\n📚 **Full requirements for ${programmeDetails.name}:**\n${programmeDetails.requirements
            .map(
              (r) =>
                `• ${r.subject.name} (${r.requirementType}${
                  r.minimumGrade
                    ? ` - Min grade: ${r.minimumGrade}`
                    : ''
                })`,
            )
            .join('\n')}`;
        ...
        return {
          message: `❌ **Why you don't qualify for ${targetProgramme}**\n\nMissing subjects:\n❌ ${notEligible.missing.join(
            '\n❌ ',
          )}\n\n💡 **To qualify for ${targetProgramme}, you would need to have these subjects.**${detailsSection}\n\n**Alternative suggestion:** Consider these programmes that better match your subjects.`,
          suggestions: [
    ...
    return {
      message: `📊 **Summary of your eligibility check**\n\n✅ **Qualify for:** ${
        lastResult.eligible.length > 0
          ? lastResult.eligible
              .map((p) => `${p.name} (${p.code})`)
              .join(', ')
          : 'None'
      }\n\n⚠️ **Partially qualify for:** ${
        lastResult.conditional.length > 0
          ? lastResult.conditional
              .map((p) => `${p.name} (${p.code})`)
              .join(', ')
          : 'None'
      }\n\n❌ **Don't qualify for:** ${
        lastResult.notEligible.length > 0
          ? lastResult.notEligible
              .map((p) => `${p.name} (${p.code})`)
              .join(', ')
          : 'None'
      ...
      suggestions: [
        ...(lastResult.notEligible.length > 0
          ? [
              `Why can't I study ${lastResult.notEligible[0].name}?`,
            ]
          : []),
        ...(lastResult.eligible.length > 0
          ? [
              `Tell me more about ${lastResult.eligible[0].name}`,
            ]
          : []),
  ...
  private async handleClarificationResponse(
    ...
    if (entities.levels.length > 0) {
      ...
      if (
        ...
        return {
          message: `Great, **${
            entities.levels[0].charAt(0) +
            entities.levels[0].slice(1).toLowerCase()
          }** level! Now, to find the right programme for you:\n\n📌 **Do you have specific subjects you studied?**\nOr\n📌 **Is there a particular career you're interested in?**`,
          suggestions: [
            'I have Biology, Chemistry and Mathematics',
      ...
      return this.handleSubjectSearch(
        entities,
        query,
        profile,
      );
    ...
    if (entities.careerKeywords.length > 0) {
      return this.handleCareerSearch(
        entities,
        query,
        profile,
      );
    ...
    if (/^yes|yeah|sure|okay|alright|please/i.test(q)) {
      if (profile.lastProgrammes.length > 0) {
        return this.handlePronounReference(
          {
            ...entities,
            pronounRef: 'first' as const,
          },
          query,
          profile,
        );
    ...
    if (/^no|nope|not really|nah/i.test(q)) {
      return {
        message:
          'No problem! Let me help you find something else. What are you interested in?\n\n• **Subjects** you studied?\n• A **career** you want to pursue?\n• A specific **faculty**?',
        suggestions: [
  ...
  private async handlePronounReference(
    entities: EntityStore,
    query: string,
    profile: UserProfile,
  ): Promise<AiResponse> {
    ...
    if (!programme) {
      return {
        message:
          'I couldn\'t find which programme you\'re referring to. Could you please specify the name or code?',
        suggestions: [
          'Tell me about the first programme',
    ...
    if (!fullProgramme) {
      return {
        ...
        suggestions: [
          'Show me the list again',
          'Tell me about another programme',
        ],
        flowState: 'CLARIFYING',
      };
    }
    ...
    return {
      message: `Here are the details for **${fullProgramme.name} (${fullProgramme.code})**:\n\n📋 **Details:**\n• **Degree:** ${
        fullProgramme.degree?.name ?? ''
      }\n• **Level:** ${fullProgramme.level.replace('_', ' ')}\n• **Duration:** ${
        fullProgramme.duration
      } years\n• **Faculty:** ${
        fullProgramme.department.academicUnit.name
      }\n• **Tuition:** ${tuitionStr}\n\n${
        fullProgramme.description
          ? `📝 **Overview:** ${fullProgramme.description.substring(
              0,
              200,
            )}...\n\n`
          : ''
      }${
        fullProgramme.requirements.length > 0
          ? `📚 **Key Requirements:** ${fullProgramme.requirements
              .slice(0, 5)
              .map((r: any) => r.subject.name)
              .join(', ')}`
          : ''
      }`,
      programmes: [
        {
          ...
          degree: fullProgramme.degree?.name ?? '',
          faculty:
            fullProgramme.department.academicUnit.abbreviation ??
            '',
  ...
  private async handleComparison(
    ...
    if (codes.length < 2) {
      return {
        message:
          'To compare programmes, please mention two or more programmes. For example: "Compare **MBBS-01** and **BSC-NUR-01**"',
        suggestions: [
          'Compare MBBS and BSc Nursing',
          'Compare Computer Science and Computer Engineering',
          'Show me differences between programmes',
        ],
        flowState: 'CLARIFYING',
      };
    }
    ...
    if (programmes.length < 2) {
      return {
        message:
          'I could only find one of those programmes. Could you check the programme codes?',
        suggestions: [
          'Compare MBBS and BSc Nursing',
          'Show me available programmes',
        ],
        flowState: 'CLARIFYING',
      };
    }
    ...
    return {
      message: `📊 **Comparison: ${programmes
        .map((p) => p.name)
        .join(' vs ')}**\n\n${programmes
        .map((p) => {
          const t = p.tuition?.[0];
          const tuitionStr = t
            ? `${Number(t.amount).toLocaleString()} XAF`
            : 'N/A';
          ...
          } requirements | 💼 ${p._count.careers} careers`;
        })
  ...
  private async handleUndecided(
    ...
    const totalProgrammes =
      await this.prisma.programme.count();
  ...
  private async handleGeneralQuery(
    ...
    const searchResults =
      await this.programmesService.search({
        query,
        limit: 4,
      });
  ...
  private async processEligibility(
    ...
    try {
      const rules =
        await this.prisma.generalAdmissionRule.findMany({
          where: {
            isActive: true,
          },
          take: 3,
      ...
      result.generalRules = rules.map(
        (r) =>
          `${r.title}: ${r.description.substring(0, 100)}`,
      );
    } catch (e) {
      this.logger.warn(
        'Could not load admission rules',
        e,
      );
    }
    ...
    const oLevelSubjects: Array<{
      subjectId: string;
      grade: string;
    }> = [];

    const aLevelSubjects: Array<{
      subjectId: string;
      grade: string;
    }> = [];

    for (const swg of subjectsWithGrades) {
      const subjectId = subjectNameToId.get(
        swg.name.toLowerCase(),
      );

      if (subjectId) {
        const subjectRec = subjectRecords.find(
          (s) => s.id === subjectId,
        );
    ...
    for (const programme of matchingProgrammes) {
      ...
      if (
        ...
        missingSubjects.length <=
          Math.ceil(
            programme.requirements.length / 2,
          ) &&
        programme.requirements.length > 0
      ) {
    ...
    result.notEligible.sort(
      (a, b) => a.missing.length - b.missing.length,
    );

    result.conditional.sort(
      (a, b) => a.missing.length - b.missing.length,
    );
  ...
  private async findProgrammesBySubjects(
    subjects: string[],
  ): Promise<any[]> {
    const subjectRecords =
      await this.prisma.subject.findMany({
    ...
    const subjectIds = subjectRecords.map(
      (s) => s.id,
    );
