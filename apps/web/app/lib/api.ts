...
class ApiClient {
  ...
  async getCatalogueReview(page = 1, limit = 20) {
    return this.request<{
      ...
      `/admin/catalogue/review?page=${page}&limit=${limit}`,
    );
  }

  async resolveCatalogueReview(id: string, input: any) {
    return this.request<{ success: boolean; data: any }>(
      `/admin/catalogue/review/${id}`,
      {
        method: 'PATCH',
        body: JSON.stringify(input),
      },
  ...
  async getAdminDepartments(params?: {
  universityId?: string;
  ...
  if (params?.academicUnitId) {
    query.set('academicUnitId', params.academicUnitId);
  }
  ...
  return this.request<{
    ...
    `/admin/departments${queryString ? `?${queryString}` : ''}`,
  );
}

  async createAdminSubject(input: {
    name: string;
    code?: string;
    level: string;
  }) {
  ...
  async deleteAdminSubject(id: string) {
   ...
   async getAdminAcademicUnitTypes() {
  return this.request<{
    success: boolean;
    data: Array<{
      id: string;
  ...
  }>('/admin/academic-unit-types');
}
  // Admin - Faculties
    async getAdminFaculties(universityId?: string) {
    const params = universityId
      ? `?universityId=${encodeURIComponent(universityId)}`
      : '';
...
async createAdminFaculty(input: any) {
  ...
  async createAdminTuition(input: {
    programmeId: string;
    academicYear: string;
    amount: number;
    currency?: string;
...
async createAdminUniversity(input: {
    name: string;
    abbreviation: string;
    description?: string;
    website?: string;
  ...
  async createAdminAdmissionRule(input: {
    title: string;
    description: string;
    isActive?: boolean;
  }) {
  ...
  // Admin - Announcements
  async createAdminAnnouncement(input: {
    title: string;
    summary?: string;
    content: string;
  ...
  async getAnnouncementCategories() {
    return this.request<{ success: boolean; data: any[] }>(
      '/admin/announcement-categories',
    );
  }

  async createAnnouncementCategory(input: {
    name: string;
    description?: string;
    slug?: string;
    displayOrder?: number;
  ...
  async toggleAnnouncementCategory(id: string) {
    return this.request<{ success: boolean; data: any }>(
      `/admin/announcement-categories/${id}/toggle`,
      {
        method: 'PATCH',
      },
    );
  ...
  async getDuplicateFaculties() {
    return this.request<{ success: boolean; data: any[] }>(
      '/admin/duplicates/faculties',
    );
  }

  async getDuplicateProgrammes() {
    return this.request<{ success: boolean; data: any[] }>(
      '/admin/duplicates/programmes',
    );
  }

  async getDuplicateSubjects() {
    return this.request<{ success: boolean; data: any[] }>(
      '/admin/duplicates/subjects',
    );
  }
  ...
  async importQuick(type: string, data: any) {
    const importData = Array.isArray(data)
      ? data
      : [data];
  ...
  async addProgrammeCareer(
    ...
    return this.request<{ success: boolean; data: any }>(
      `/admin/programmes/${programmeId}/careers`,
      {
        method: 'POST',
  ...
  async removeProgrammeCareer(
    ...
    return this.request<{ success: boolean; data: any }>(
      `/admin/programmes/${programmeId}/careers/${careerId}`,
      {
        method: 'DELETE',
      },
    );
  ...
  async addProgrammeKeyword(
    ...
    return this.request<{ success: boolean; data: any }>(
      `/admin/programmes/${programmeId}/keywords`,
      {
        method: 'POST',
  ...
  async removeProgrammeKeyword(
    ...
    return this.request<{ success: boolean; data: any }>(
      `/admin/programmes/${programmeId}/keywords/${keywordId}`,
      {
        method: 'DELETE',
      },
    );
  ...
  async createProgrammeRequirement(
    programmeId: string,
    input: {
      subjectId: string;
  ...
  async bulkAddRequirements(
    ...
    }>(`/programmes/${programmeId}/requirements/bulk`, {
      method: 'POST',
  ...
  async getProgrammesByFaculty() {
    return this.request<{ success: boolean; data: any[] }>(
      '/admin/dashboard/programmes-by-faculty',
    );
  }

  async getPopularSearches(limit = 10) {
    return this.request<{ success: boolean; data: any[] }>(
      `/admin/dashboard/popular-searches?limit=${limit}`,
    );
  }

  async getRecentSearches(limit = 20) {
    return this.request<{ success: boolean; data: any[] }>(
      `/admin/dashboard/recent-searches?limit=${limit}`,
    );
  }
