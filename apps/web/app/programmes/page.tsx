...
function getDegreeName(degree: unknown): string {
  if (!degree) {
    return "Programme";
  }
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      {isLoading ? (
        view === "grid" ? (
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {[1, 2, 3, 4, 5, 6].map((item) => (
              <ProgrammeCardSkeleton key={item} />
            ))}
          </div>
        ) : (
          <div className="space-y-3">
            {[1, 2, 3, 4].map((item) => (
              <ProgrammeListSkeleton key={item} />
            ))}
          </div>
        )
      ) : isError ? (
        ...
        <EmptyState
          ...
          description={
            debouncedQuery
              ? `No programmes matching "${debouncedQuery}". Try different search terms or filters.`
                ...
                <CardContent className="p-5">
                  ...
                  <div className="mt-3 space-y-2">
                    ...
                    <div className="flex items-center gap-2 text-sm text-zinc-500 dark:text-zinc-400">
                      ...
                      {prog.duration}{" "}
                      {prog.duration === 1 ? "year" : "years"}
                    </div>
                  </div>

                  <p className="mt-3 line-clamp-2 text-sm text-zinc-500 dark:text-zinc-400">
                    {prog.description ||
                      "A comprehensive programme designed to equip students with relevant knowledge and skills."}
                  </p>
                </CardContent>
              </Card>
            </Link>
    ...
    <Suspense
      fallback={
        <div className="mx-auto max-w-7xl px-4 py-20 text-center">
          <p className="text-zinc-500">Loading programmes...</p>
        </div>
      }
    >
      <ProgrammesContent />
